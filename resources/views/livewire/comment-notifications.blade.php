    ...
    >
        @if ($notifications->isNotEmpty() && ! $isLoading)
            @foreach ($notifications as $notification)
                <li>
                    <a class="flex hover:bg-gray-100 transition duration-150 ease-in px-5 py-3"
            ...
            @foreach (range(1,3) as $item)
                <li class="animate-pulse flex items-center transition duration-150 ease-in px-5 py-3">
                    ...
                    <div class="flex-1 ml-4 space-y-2">
                        <div class="bg-gray-300 w-full rounded h-4"></div>
                        <div class="bg-gray-300 w-full rounded h-4"></div>
                        <div class="bg-gray-300 w-1/2 rounded h-4"></div>
                    </div>
                </li>
            @endforeach
        @else
